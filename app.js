...
(function () {
  ...
  const SUPABASE_ANON = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6IndkbXBnZWVnemJ6YWFmaHdqcWF6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTUxNjU3NDMsImV4cCI6MjA3MDc0MTc0M30.mNcriLvMZJ6jWUutuMiXrA4PNrvmV1JmzIgxkGP3d0U'; // <-- your anon JWT
  ...
  function showTextModal(title, text){
    ...
    if(!wrap){
      ...
      btnCopy.onclick = async ()=>{ try{ await navigator.clipboard.writeText(pre.textContent); alert('Copied to clipboard'); }catch{} };
  ...
  async function refreshSessionUI(){
    ...
    if($('roleIndicator')) $('roleIndicator').textContent = user ? `Signed in as ${user.email||'Discord user'}` : 'Welcome';
    if($('loginBtn'))  $('loginBtn').style.display  = user ? 'none' : '';
    if($('logoutBtn')) $('logoutBtn').style.display = user ? '' : 'none';
    if($('emailInput')) $('emailInput').style.display = 'none'; // we use Discord OAuth now
  ...
  sb.auth.onAuthStateChange((_event, _session)=>{ refreshSessionUI(); loadProfiles(); });

  // ====== DATA ======
  async function loadProfiles(){
    const tbody = $('profilesBody');
    if(!tbody) return;
    tbody.innerHTML = '<tr><td colspan="12" style="text-align:center;color:var(--muted);padding:24px">Loading…</td></tr>';

    const { data, error } = await sb.from('profiles').select('*').order('created_at', { ascending:false });
    if (error){
      tbody.innerHTML = `<tr><td colspan="12" class="muted" style="text-align:center;padding:24px">${escapeHtml(error.message)}</td></tr>`;
      return;
    }

    const term = ($('searchBar')?.value||'').toLowerCase();
    const rows = (data||[]).filter(p => {
      const t=[p.nickname,p.screen_hz,p.headphones,p.mouse,p.keyboard,p.dpi,p.sens,p.zoom,(p.cfg_name||''),(p.pak_name||'')].join(' ').toLowerCase();
      return !term || t.indexOf(term)!==-1;
    });

    if(!rows.length){
      tbody.innerHTML = '<tr><td colspan="12" class="muted" style="text-align:center;padding:24px">No matches.</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = rows.map(p => {
      ...
      const pakCell = p.pak_url
        ? '<a class="secondary" href="'+escapeHtml(p.pak_url)+'" download="'+escapeHtml(p.pak_name||"pak")+'">Download</a>'
        : '<span class="muted">—</span>';
      ...
      return '<tr>'+
        ...
        '<td>'+cfgCell+'</td>'+
        '<td>'+pakCell+'</td>'+              // <-- NEW: always render the Paks cell
  ...
  if(searchBar) searchBar.addEventListener('input', loadProfiles);

  // Delegated table actions
  const bodyEl = $('profilesBody');
  if(bodyEl){
  ...
  async function openPlayer(id){
    ...
    const modal = $('playerModal'); if(modal) modal.classList.add('open');
  }
  const closePlayer = $('closePlayer');
  if(closePlayer) closePlayer.addEventListener('click', ()=> $('playerModal').classList.remove('open'));
  ...
  function openEdit(id){
    if($('editTitle')) $('editTitle').textContent = id? 'Edit Player' : 'Add Player';
    if($('e_id')) $('e_id').value = id||'';
    if($('e_nick')) $('e_nick').value = '';
    ['screen','head','mouse','keyboard','dpi','sens','zoom','country','clan','map','about'].forEach(k=> { const el=$('e_'+k); if(el) el.value=''; });
    ...
    if(id){
      sb.from('profiles').select('*').eq('id', id).single().then(({data:p,error})=>{
        ...
        if($('e_nick'))     $('e_nick').value     = p.nickname||'';
        if($('e_screen'))   $('e_screen').value   = p.screen_hz||'';
        if($('e_head'))     $('e_head').value     = p.headphones||'';
        if($('e_mouse'))    $('e_mouse').value    = p.mouse||'';
        if($('e_keyboard')) $('e_keyboard').value = p.keyboard||'';
        if($('e_dpi'))      $('e_dpi').value      = p.dpi||'';
        if($('e_sens'))     $('e_sens').value     = p.sens||'';
        if($('e_zoom'))     $('e_zoom').value     = p.zoom||'';
        if($('e_country'))  $('e_country').value  = p.country||'';
        if($('e_clan'))     $('e_clan').value     = p.clan||'';
        if($('e_map'))      $('e_map').value      = p.favorite_map||'';
        if($('e_about'))    $('e_about').value    = p.about||'';
      });
    }
    const modal=$('editModal'); if(modal) modal.classList.add('open');
  }

  const btnRemovePic = $('btnRemovePic');
  if(btnRemovePic){
  ...
  const btnRemoveCfg = $('btnRemoveCfg');
  if(btnRemoveCfg){
  ...
  const btnRemovePak = $('btnRemovePak');
  if(btnRemovePak){
  ...
  if(editForm){
    editForm.addEventListener('submit', async (e)=>{
      ...
      try{
        ...
        // handle pak upload (zip/rar/7z up to 50MB)
        if(pakFile){
  ...
  // ====== INIT ======
  async function init(){ loadHeaderLogo(); await refreshSessionUI(); loadProfiles(); }
  init();
